...
function plot_ES_VaR_bootstrap(df, n, B)
      ...
      % 1. Data Generation
        ...
        beta = -0.3;   % Skewness parameter (-1 <= beta <= 1)
        scale = 1;    % Scale parameter (gamma > 0)
        delta = 0;    % Location parameter (real number)
      
        
        % Simulate data from the stable Paretian distribution
        data = stabgen(n, alpha, beta, scale, delta);

              % Initial guess for parameters
        initvec = [1.5, 0, 1, 0]; % Initial guess for [alpha, beta, gamma, delta]
        vlo = 0.1; % Minimum alpha value
        fixd = []; % Estimate all parameters

    % Estimate parameters
        [param, ~, ~, ~, ~] = ASPEstimation(data, vlo, initvec, fixd);
    ...
    fprintf('True Expected Shortfall (ES): %.4f\n', actual1);
        ...
        [VaR_Pareto, ES_Pareto] = compute_ES_Pareto_Bootstrap(data, B, n, gamma);
        ...
        VaR_data = [VaR_nonparametric_values, VaR_gaussian_values, VaR_parametric_values, VaR_ES_GA_t, VaR_ES_NCT,VaR_mixture_values, VaR_Pareto];
        VaR_labels = {'Non-Parametric', 'Gaussian', 'Student t','ESGAt','NCT', 'Mixture of Normals', 'Pareto'};
        
        % Prepare data for ES Boxplot
        ES_data = [ES_nonparametric_values, ES_gaussian_values, ES_parametric_values, ES_GA_t,ES_NCT, ES_mixture_values, ES_Pareto];
        ES_labels = {'Non-Parametric', 'Gaussian', 'Student t','ESGAt','NCT' ,'Mixture of Normals', 'Pareto'};
...
function [VaR_values, ES_values] = compute_ES_Pareto_Bootstrap(data, B, n, gamma)
    ...
    for b = 1:B
        fprintf('Starting Pareto estimation for bootstrap sample %d out of %d...\n', b, B);
        % Generate bootstrap sample
        ind = unidrnd(n, [n, 1]);
        bootsamp = data(ind);
      ...
      % Initial guess for parameters
        ...
        % Estimate parameters
        [param, ~, ~, ~, ~] = ASPEstimation(bootsamp, vlo, initvec, fixd);
            estimates = param;
            alpha = estimates(1);
            beta = estimates(2);
            scale = estimates(3);
            location = estimates(4); 
            fprintf('Parameters %.4f\n', param);
    
            % Compute VaR
            [actual1, actual2] = asymstableES(gamma, alpha, beta, location, scale, 1);
    
            % Compute ES using numerical integration
            VaR_values(b) = actual2;
            ES_values(b) = actual1;
   ...
   fprintf('ES value for bootstrap sample %d: %f\n', b, ES_values(b));
   fprintf('VaR value for bootstrap sample %d: %f\n', b, VaR_values(b));
    end
...
function [param, stderr, iters, loglik, Varcov] = ASPEstimation(data, vlo, initvec, fixd)
    % ASPEstimation estimates parameters of the asymmetric stable Paretian distribution.
    % [param, stderr, iters, loglik, Varcov] = ASPEstimation(data, vlo, initvec, fixd)
    % - `data`: Observed data to fit.
    % - `vlo`: Minimum value for the stability parameter alpha.
    % - `initvec`: Initial guess for parameters [alpha, beta, gamma, delta].
    % - `fixd`: Fixed delta parameter (optional).

    if nargin < 2, vlo = 0.1; end
    if nargin < 3, initvec = []; end
    if nargin < 4, fixd = []; end

    if isempty(initvec)
        % Default initial parameter guesses
        initvec = [1.5, 0, 1, 0]; % [alpha, beta, gamma, delta]
    end

    % Bounds for the parameters
    if isempty(fixd)
        bound.lo = [vlo, -1, 1e-4, -Inf]; % Lower bounds
        bound.hi = [2, 1, 1e4, Inf];      % Upper bounds
        bound.which = [1, 1, 1, 1];       % Which parameters are estimated
    else
        bound.lo = [vlo, -1, 1e-4];       % Lower bounds (delta fixed)
        bound.hi = [2, 1, 1e4];          % Upper bounds (delta fixed)
        bound.which = [1, 1, 1];         % Which parameters are estimated
    end

    % Optimization options
    opts = optimset('Display', 'off', 'MaxIter', 500, 'TolFun', 1e-8, ...
                    'TolX', 1e-8, 'MaxFunEvals', 2000);

    % Optimize parameters
    [pout, fval, exitflag, output, grad, hess] = fminunc(...
        @(param) ASPLogLik(param, data, fixd, bound), ...
        einschraenk(initvec, bound), opts);

    % Compute Variance-Covariance matrix
    nobs = length(data);
    V = inv(hess) / nobs;
    [param, V] = einschraenk(pout, bound, V);

    % Outputs
    param = param';
    Varcov = V;
    stderr = sqrt(diag(V));
    loglik = -fval * nobs; % Log-likelihood at maximum
    iters = output.iterations;
end

function ll = ASPLogLik(param, data, fixd, bound)
    % Negative log-likelihood function for asymmetric stable Paretian
    if nargin < 4, bound = 0; end
    if isstruct(bound)
        paramvec = einschraenk(real(param), bound, 999);
    else
        paramvec = param;
    end
    ...
    % Evaluate PDF
    pdf = ASPPDF(z, alpha, beta) / gamma;
    llvec = log(pdf);
    ll = -mean(llvec); % Return negative log-likelihood
    if isinf(ll), ll = 1e5; end
end

function pdf = ASPPDF(x, alpha, beta)
    % PDF of the asymmetric stable Paretian distribution
    tol = 1e-8;
    lo = tol;
    hi = 1 - tol;
    ...
    for i = 1:length(x)
        xi = x(i);
        pdf(i) = integral(@(u) ASPIntHelper(u, xi, alpha, beta), lo, hi, ...
                          'RelTol', 1e-6, 'AbsTol', 1e-8) / pi;
    end
end

function I = ASPIntHelper(u, x, alpha, beta)
    % Helper function for integration
    t = (1 - u) ./ u;
    if alpha == 1
        cf = exp(-abs(t) .* (1 + 1i * beta * (2 / pi) .* sign(t) .* log(t)));
    else
        cf = exp(-(abs(t).^alpha) .* (1 - 1i * beta * sign(t) * tan(pi * alpha / 2)));
    end
    z = exp(-1i * t * x) .* cf;
    I = real(z) .* u.^(-2); % PDF component
end
...
% Step 1: Define the stabgen function
function x = stabgen(nobs, a, b, c, d)
    % Simulate data from a stable distribution
    if nargin < 3, b = 0; end
    if nargin < 4, c = 1; end
    if nargin < 5, d = 0; end
    z = nobs;
    ...
    W = exprnd(1, 1, z);
    if a == 1
        x = (2 / pi) * (((pi / 2) + b * V) .* tan(V) - ...
            b * log((W .* cos(V)) ./ ((pi / 2) + b * V)));
...
% Step 2: Define the log-likelihood function for stable distribution
function logL = log_likelihood_stable(params, data)
    % Log-likelihood for stable distribution
    a = params(1); b = params(2); c = params(3); d = params(4);
    
    % Parameter constraints
    if a <= 0 || a > 2 || c <= 0 || abs(b) > 1
        logL = -Inf;
        return;
    end
    ...
    % Compute the PDF
    [pdf, ~] = asymstab(xvec, a, b); % Call asymstab to compute PDF
    if any(pdf <= 0) % Ensure the PDF is valid
        logL = -Inf;
        return;
    end
    
    % Compute the log-likelihood
    logL = sum(log(pdf)) - length(data) * log(c); % Include scale adjustment
end
